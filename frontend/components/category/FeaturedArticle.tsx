...
export function FeaturedArticle({ article, categoryName }: Props) {
  ...
  return (
    <article className={`flex flex-col border-b border-border pb-6 ${isRead ? "opacity-60" : ""}`}>
      ...
      <h2 className="mt-2.5 text-2xl font-black leading-tight tracking-tight">
        <a
          href={article.link}
          target="_blank"
          ...
          onClick={handleClick}
          className="outline-none hover:text-brand focus-visible:text-brand focus-visible:ring-2 focus-visible:ring-brand"
        >
          {article.title}
        </a>
      </h2>
